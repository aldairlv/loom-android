...
sealed interface PostsUiState {
    ...
    data class Error(val message: String? = null) : PostsUiState
}
@HiltViewModel
class PostsViewModel @Inject constructor(
    ...
    fun fetchPosts(isLoadMore: Boolean = false) {
        ...
        viewModelScope.launch {
            if (!isLoadMore) {
                ...
                if (current is PostsUiState.Success) {
            ...
            when (val result = profileRepository.getMyPosts(cursorToPass)) {
                is Result.Success -> {
    ...
    fun loadMore() {
        ...
        if (state is PostsUiState.Success && !state.isFetchingMore && currentCursor != null) {
